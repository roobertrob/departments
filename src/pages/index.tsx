import LoggedContainer from "Layout/Wrappers/LoggedContainer";
...
const Home = () => {
   ...
   return (
      <LoggedContainer
         ...
         subtitulo='Veja aqui quais são:'
         classe={classe}
      >
         <div className={`sm:w-1/4 w-full`}>
            <div className='flex flex-col mb-2'>
               <input
                  className={`px-4 py-3 rounded-lg bg-gray-200 text-gray-900 mt-2 border focus:border-blue-400 focus:outline-none focus:bg-white`}
         ...
         <table
            className={`sm:w-1/4 min-w-min flex flex-col w-full border rounded-lg`}
         >
            <thead className={``}>
               <tr>
                  ...
                  ? filteredResults.map((departamentoFiltrado) => {
                       return (
                          <tr key={departamentoFiltrado.id}>
                             <td
                                className={`flex items-center justify-center mr-1 p-3 border `}
                             >
                                {departamentoFiltrado.id}
                             </td>
                  ...
                  : todosDeps.map((departamento) => {
                       return (
                          <tr key={departamento.id}>
                             <td
                                className={`flex items-center justify-center mr-1 p-3 border rounded-lg`}
                             >
                                {departamento.id}
                             </td>
